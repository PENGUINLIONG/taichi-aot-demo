...
public:
  DrawMeshBuilder(
    ...
    assert(positions.elem_shape().dims[0] > 0 && positions.elem_shape().dims[0] <= 4);
    assert(indices.shape().dim_count == 1);
    assert(indices.shape().dims[0] != 0);
    assert(indices.elem_shape().dim_count == 1);
    assert(indices.elem_shape().dims[0] == 3);
