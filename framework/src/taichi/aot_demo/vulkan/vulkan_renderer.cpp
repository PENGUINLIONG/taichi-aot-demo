...
Renderer::Renderer(const RendererConfig &config)
  ...
  vrii.api_version = state_->api_version_;
  vrii.instance = state_->instance_;
  vrii.physical_device = state_->physical_device_;
  vrii.device = state_->device_;
  vrii.compute_queue = state_->queue_;
  vrii.compute_queue_family_index = state_->queue_family_index_;
  vrii.graphics_queue = state_->queue_;
  vrii.graphics_queue_family_index = state_->queue_family_index_;
  ti::Runtime runtime =
...
void RendererState::set_swapchain() {
  VkResult res = VK_SUCCESS;
  ...
  vkGetSwapchainImagesKHR(device_, swapchain, &nswapchain_image,
...
#if TI_AOT_DEMO_GLFW
void RendererState::set_surface_window(GLFWwindow* window) {
  VkResult res = VK_SUCCESS;
...
void RendererState::set_surface_window(ANativeWindow* window) {
  VkResult res = VK_SUCCESS;
...
void RendererState::set_framebuffer_size(uint32_t width, uint32_t height) {
  VkResult res = VK_SUCCESS;
  assert(!in_frame_);
...
void Renderer::begin_render() {
  ...
  cbai.commandPool = state_->command_pool_;
  cbai.commandBufferCount = 1;

  VkCommandBuffer command_buffer = VK_NULL_HANDLE;
  res = vkAllocateCommandBuffers(state_->device_, &cbai, &command_buffer);
  check_vulkan_result(res);
  ...
  std::array<VkImageMemoryBarrier, 2> imbs {};
  {
    ...
    caimb.image = state_->color_attachment_;
  ...
  }
  {
    ...
    daimb.image = state_->depth_attachment_;
  ...
  rpbi.renderPass = state_->render_pass_;
  rpbi.framebuffer = state_->framebuffer_;
  rpbi.renderArea.extent.width = state_->width_;
  rpbi.renderArea.extent.height = state_->height_;
...
void Renderer::end_render() {
  ...
  vkCmdEndRenderPass(state_->frame_command_buffer_);

  res = vkEndCommandBuffer(state_->frame_command_buffer_);
  check_vulkan_result(res);

  // Ensure image transitions are submitted.
  runtime_.flush();

  VkSubmitInfo si {};
  si.sType = VK_STRUCTURE_TYPE_SUBMIT_INFO;
  si.commandBufferCount = 1;
  si.pCommandBuffers = &state_->frame_command_buffer_;
  si.signalSemaphoreCount = 1;
  si.pSignalSemaphores = &state_->render_present_semaphore_;
  res = vkQueueSubmit(state_->queue_, 1, &si, nullptr);

  state_->in_frame_ = false;
  state_->frame_command_buffer_ = VK_NULL_HANDLE;
}
void Renderer::enqueue_graphics_task(const std::shared_ptr<GraphicsTask> &graphics_task) {
  assert(state_->in_frame_);

  const GraphicsTaskConfig &config = graphics_task->state()->config_;
  VkCommandBuffer cmdbuf = state_->frame_command_buffer_;

  // Fetch data to shadow memory objects.
  if (config.vertex_buffer != nullptr) {
    config.vertex_buffer->update();
  }
  if (config.index_buffer != nullptr) {
    config.index_buffer->update();
  }
  for (const GraphicsTaskResource &rsc : config.resources) {
    switch (rsc.type) {
  ...
  VkViewport v {};
  v.width = state_->width_;
  v.height = state_->height_;
  v.maxDepth = 1.0f;
  vkCmdSetViewport(cmdbuf, 0, 1, &v);

  VkRect2D r {};
  r.extent.width = state_->width_;
  r.extent.height = state_->height_;
  vkCmdSetScissor(cmdbuf, 0, 1, &r);

  vkCmdBindPipeline(cmdbuf, VK_PIPELINE_BIND_POINT_GRAPHICS,
    graphics_task->state_->pipeline_);

  vkCmdBindDescriptorSets(cmdbuf, VK_PIPELINE_BIND_POINT_GRAPHICS,
    graphics_task->state_->pipeline_layout_, 0, 1,
    &graphics_task->state_->descriptor_set_, 0, nullptr);

  {
    const TiVulkanMemoryInteropInfo& vmii = state_->export_ti_memory(runtime_, *config.vertex_buffer);
    VkDeviceSize o = 0;
    vkCmdBindVertexBuffers(cmdbuf, 0, 1, &vmii.buffer, &o);
  }

  if (is_indexed) {
    const TiVulkanMemoryInteropInfo& vmii = state_->export_ti_memory(runtime_, *config.index_buffer);
    vkCmdBindIndexBuffer(cmdbuf, vmii.buffer, 0, VK_INDEX_TYPE_UINT32);

    vkCmdDrawIndexed(cmdbuf, config.index_count, config.instance_count, 0, 0, 0);
  } else {
    vkCmdDraw(cmdbuf, config.vertex_count, config.instance_count, 0, 0);
  }
}

void Renderer::present_to_surface() {
  assert(state_->swapchain_);
  VkResult res = VK_SUCCESS;

  VkDevice device = state_->device_;

  uint32_t i = !0u;
  res = vkAcquireNextImageKHR(device, state_->swapchain_, 0, VK_NULL_HANDLE, state_->acquire_fence_, &i);
  check_vulkan_result(res);

  res = VK_TIMEOUT;
  while (res > VK_SUCCESS) {
    res = vkWaitForFences(device, 1, &state_->acquire_fence_, VK_TRUE, 1000000000);
    check_vulkan_result(res);
  }

  vkResetFences(device, 1, &state_->acquire_fence_);
  check_vulkan_result(res);

  VkImage swapchain_image = state_->swapchain_images_.at(i);
  ...
  ib.srcOffsets[1].x = state_->width_;
  ib.srcOffsets[1].y = state_->height_;
  ib.srcOffsets[1].z = 1;
  ib.dstSubresource.aspectMask = VK_IMAGE_ASPECT_COLOR_BIT;
  ib.dstSubresource.layerCount = 1;
  ib.dstOffsets[1].x = state_->swapchain_image_width_;
  ib.dstOffsets[1].y = state_->swapchain_image_height_;
  ib.dstOffsets[1].z = 1;
  vkCmdBlitImage(command_buffer,
    state_->color_attachment_, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
  ...
  si.pWaitSemaphores = &state_->render_present_semaphore_;
  si.pWaitDstStageMask = &ps;
  res = vkQueueSubmit(state_->queue_, 1, &si, state_->present_fence_);
  check_vulkan_result(res);

  VkPresentInfoKHR pi {};
  pi.sType = VK_STRUCTURE_TYPE_PRESENT_INFO_KHR;
  pi.swapchainCount = 1;
  pi.pSwapchains = &state_->swapchain_;
  pi.pImageIndices = &i;
  pi.pResults = &res;
  res = vkQueuePresentKHR(state_->queue_, &pi);
  check_vulkan_result(res);
}
...
ti::NdArray<uint8_t> Renderer::present_to_ndarray() {
  ...
  bic.imageExtent.width = state_->width_;
  bic.imageExtent.height = state_->height_;
  bic.imageExtent.depth = 1;
  bic.imageSubresource.aspectMask = VK_IMAGE_ASPECT_COLOR_BIT;
  bic.imageSubresource.layerCount = 1;
  vkCmdCopyImageToBuffer(command_buffer, state_->color_attachment_, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, vmii.buffer, 1, &bic);
...
void Renderer::next_frame() {
  ...
  res = VK_TIMEOUT;
  while (res > VK_SUCCESS) {
    res = vkWaitForFences(device, 1, &state_->present_fence_, VK_TRUE, 1000000000);
    check_vulkan_result(res);
  }

  res = vkResetFences(device, 1, &state_->present_fence_);
  check_vulkan_result(res);

  vkResetCommandPool(device, state_->command_pool_, 0);
  check_vulkan_result(res);

  staging_buffers_.clear();
}

const TiVulkanMemoryInteropInfo &RendererState::export_ti_memory(
  ...
  if (it == ti_memory_interops_.end()) {
    TiVulkanMemoryInteropInfo vmii {};
    ti_export_vulkan_memory(runtime, memory, &vmii);
    check_taichi_error();
...
const TiVulkanImageInteropInfo &RendererState::export_ti_image(
    const ti::Runtime& runtime,
  ...
  if (it == ti_image_interops_.end()) {
    ...
    ti_export_vulkan_image(runtime, image, &viii);
    check_taichi_error();
